...
_MODE = "competition"
_EXTERNAL_BROKER: Path | None = None
...
def init_ctx(workspace: Path, slug: str, budget, trace,
             ...
             external_broker: Path | None = None) -> None:
    global _CTX, _MODE, _EXTERNAL_BROKER
    _CTX = R.Ctx(workspace=workspace, slug=slug, budget=budget, trace=trace)
    _MODE = mode
    _EXTERNAL_BROKER = Path(external_broker) if external_broker else None
...
def make_server(solver_id: str):
    ...
    for item in _KAGGLE:
        name = item[0]
        if _MODE == "clean-benchmark" and name == "kaggle_submissions":
            continue
        # external-broker 模式下 Claude 进程没有 Kaggle 凭证；所有可变外部
        # 操作只能由总 Broker 执行。保留只读 memory/skill 工具。
        if _EXTERNAL_BROKER and name.startswith("kaggle_"):
            continue
    ...
    if not _EXTERNAL_BROKER:
        tools.append(gpu_kernel_lease)
    ...
    if not _EXTERNAL_BROKER:
        names.append("mcp__ioai__gpu_kernel_lease")
    return server, names
