...
def external_context_contract(args, *, evaluator: bool = False) -> str:
    ...
    bundle = (
        "`context/SEARCH_BUNDLE/`（Search 研究材料。可能在运行中送达：若目前为空，"
        "每轮开始时再查看一次；`ARRIVAL.md` 出现即为已送达，先核对资产再决定是否采用）"
...
#: Rounds this harness cut short itself. A watchdog interrupt surfaces through
#: the SDK as an errored result, indistinguishable from a real outage at the
#: call site — and dryrun7 spent 31 of its 33 minutes in exponential backoff
#: because our own interrupts were being counted as the service failing, with
#: solver_a and solver_b four strikes from the eight-in-a-row permanent stop
#: while Azure answered 200 throughout. Backing off is right for a rate limit
#: and wrong for a key we pressed ourselves.
            ...
            async with ClaudeSDKClient(options=o) as client:
                for rnd in range(1, args.rounds + 1):
                    ...
                    try:
                        ...
                        usage = None
                    if INTERRUPTED.get(solver):
                        # We reclaimed the turn on purpose; the round is spent
                        # but the service is fine. Go straight to the next one.
                        interrupts += 1
                        print(f"  [{solver}] round {rnd} reclaimed by the "
                              f"watchdog ({interrupts} this run) — continuing",
                              flush=True)
                        trace.log("watchdog_reclaim", solver=solver, round=rnd,
                                  count=interrupts)
                        if interrupts == 2:
                            # Twice is a working style, not bad luck: a
                            # foreground command that runs longer than the
                            # stall window will always be cut.
                            S.flag_urgent(
                                "env",
                                "你的轮次已被看门狗回收两次：轮内静默超过 "
                                "stall 窗口就会被打断。长任务请用 "
                                "`nohup cmd > log 2>&1 &` 后台跑，再 "
                                "`sleep 30; tail log` 轮询，不要前台阻塞等待。",
                                src="harness")
                        continue
...
2. `folds.json`：本轮所有候选共用的唯一计分划分。怎么切完全取决于本题，
   ...
   泄漏。harness 强制四点：文件合法
   （`{py} -m native.scripts.checkfolds --workspace {ws}`）、**至少 3 个计分折
   （fold >= 0 的不同取值 >= 3）**、**至少 60% 的单元参与计分（fold >= 0）**、
   并且一旦有候选在其上计分就永不重切。不满足会被 freeze_contract 直接拒绝。

   为什么是硬要求：这把尺子要给几十个候选排序，而每个 solver 都在对它优化。
   单一 holdout 上"分数最高"等于"对这一折拟合最狠"，实测出现过本地分与
   Public LB 完全反序（6 对里 5 对反转），系统据此选出了当轮最差的提交。
   k 折不是形式，是让排序不被单次切分的噪声和过拟合吃掉。
