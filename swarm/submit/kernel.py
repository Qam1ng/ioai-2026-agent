...
def unique_kernel_id(
    ...
    # Candidate ids can already be near Kaggle's 44-character limit. Preserve
    # the per-submission discriminator and truncate the candidate before
    # calculating the competition prefix; otherwise a negative head_budget
    # leaves an overlong id that make_metadata silently truncates to a
    # different ref, and polling waits forever on a kernel that cannot exist.
    minimum_head = 4
    tail_budget = MAX_KERNEL_SLUG - minimum_head - len(digest) - 2
    if disc:
        disc = disc[-min(len(disc), max(1, tail_budget // 2)):]
        cand_budget = max(1, tail_budget - len(disc) - 1)
        tail = f"{cand[:cand_budget].strip('-') or 'c'}-{disc}"
    else:
        tail = cand[:tail_budget].strip("-") or "c0"
    head_budget = MAX_KERNEL_SLUG - len(digest) - len(tail) - 2
    head = comp[: max(4, head_budget)].strip("-")
    kernel_slug = f"{head}-{digest}-{tail}"
    if len(kernel_slug) > MAX_KERNEL_SLUG:  # defensive invariant
        raise ValueError("kernel slug construction exceeded Kaggle's limit")
