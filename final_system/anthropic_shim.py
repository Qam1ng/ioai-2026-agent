...
#: Models actually deployed on this Azure gateway. Anything else 404s with
#: DeploymentNotFound. Claude Code picks model names from at least three places
#: — the CLI flag, CLAUDE_CODE_SUBAGENT_MODEL, and its internal opus/sonnet/
#: haiku aliases — and pinning all three still left requests arriving as
#: claude-opus-4-8 in task 4. The shim is the one chokepoint every request must
#: pass, so the mapping belongs here rather than in another env var.
DEPLOYED_MODELS = frozenset({"claude-opus-5", "claude-fable-5"})
MODEL_FALLBACK = "claude-opus-5"

DROP_FIELDS = frozenset({
    "context_management", "mcp_servers", "container", "advisor",
    # Azure: "structured_outputs not supported in your workspace". The wire
    # field is `output_config` (the container Claude Code puts its structured-
    # output request in). Claude Code already degrades gracefully when the
    # field is rejected; stripping it here saves the wasted 400 round-trip on
    # every session that tries it.
    "output_config",
})
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802 - stdlib hook
            ...
            try:
                ...
                # web_search is executed server-side by Anthropic and Azure does
                # not offer it, so every call came back 400 "not supported in
                # your workspace" — six of them in the first four minutes of
                # task 4. Removing the tool from the request makes Claude Code
                # treat it as unavailable and reach for WebFetch instead, which
                # runs client-side from the sandbox and does work (verified:
                # HTTP 200 to the open internet from inside bwrap).
                model = payload.get("model")
                if isinstance(model, str) and model not in DEPLOYED_MODELS:
                    payload["model"] = MODEL_FALLBACK
                    dropped = dropped + [f"model:{model}->{MODEL_FALLBACK}"]
                # Deliberately NOT stripping the web_search tool. Azure
                # refuses it with "not supported in your workspace", which
                # is a per-tool refusal the model routes around. Removing it
                # was worse: the conversation history still holds web_search
                # tool_use blocks, so the gateway rejected the whole request
                # with "Tool 'web_search' not found in provided tools" — a
                # request-level failure instead of a tool-level one.
                if dropped:
