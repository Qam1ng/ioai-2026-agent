...
class SystemConfig:
    ...
    def load(cls, path: Path, *, repo_root: Path) -> "SystemConfig":
        ...
        value = cls(
            run=RunConfig(
                ...
                kernel_version_budget=int(
                    run.get("kernel_version_budget", 0)
                ),
                kernel_timeout_seconds=float(
    ...
    def validate(self) -> None:
        ...
        if self.selection.calibration_min_feedback < 1:
            raise ValueError("calibration_min_feedback must be at least 1")
