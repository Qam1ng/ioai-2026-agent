...
class SelectionManager:
    """Codex advisory layer. It can recommend, but it cannot submit."""
...
你使用 Codex + {self.model}，effort={self.effort}。你只有建议权，没有 Kaggle
...
"""
    ...
    async def recommend(self, context: dict) -> dict | None:
        ...
        if result.exit_code != 0:
            errors.append(f"Codex exit={result.exit_code}")
        if result.timed_out:
            errors.append("Selection Manager timed out")
        if result.resolved_model is not None and result.resolved_model != self.model:
            errors.append(
