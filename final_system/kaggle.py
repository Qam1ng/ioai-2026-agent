...
class KaggleAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        # Some tasks cap notebook *versions*, not submissions: every push spends
        # one even if the run errors. 0 disables the cap.
        self.kernel_version_budget = int(kernel_version_budget or 0)
        ...
        # Observed push->COMPLETE wall-clock, keyed by accelerator. The declared
        # estimate is a cold guess (default 10 min CPU); once real kernels land
        # it is wildly off — a potato kernel that only base64-encodes finished
        # in ~20s while the estimate stayed 10 min and blocked the last two
        # submissions of task 3 at the deadline. Prefer measured reality.
        self._observed_minutes: dict[str, list[float]] = {"cpu": [], "gpu": []}
    ...
    def _runtime_minutes(self, record: dict) -> float:
        ...
        # Once we have measured this accelerator, trust the data over the guess.
        # Use the median of observed wall-clocks x1.5 safety margin, but never
        # exceed the declared budget (an agent may legitimately raise it for a
        # genuinely longer job). This unblocks trivial kernels near a deadline
        # without letting a real long job sneak past the account/deadline gate.
        seen = self._observed_minutes.get(accelerator) or []
        if len(seen) >= 1:
            ordered = sorted(seen)
            median = ordered[len(ordered) // 2]
            measured = min(declared, median * 1.5)
            return max(0.2, measured)
        return declared
    ...
    def _resolve_datasets(self, refs: list) -> list:
        ...
        for raw in refs:
            ...
            placeholder = bool(re.search(r"REPLACE|PLACEHOLDER|<|>", owner, re.I))
            verdict = self._dataset_exists(ref) if slug and not placeholder else False
            if slug and not placeholder and verdict is not False:
                # Verified, or unverifiable — either way keep what the agent
                # asked for rather than dropping a mount we cannot check.
                out.append(ref)
                continue
            if slug and not placeholder:
                # A wrong-but-plausible owner is worse than a placeholder:
                # Kaggle mounts nothing and says nothing, so the kernel only
                # fails minutes later with "wheels not found under
                # /kaggle/input". codex wrote "ioai-official/..." instead of
                # "kamalkhan/..." and burned three pushes on it in one run,
                # each time reading a runtime error that never named the cause.
                print(f"[broker] dataset {ref!r} does not exist; resolving by slug",
                      flush=True)
            want = slug or owner
            hit = None
            try:
            ...
            if hit:
                ...
                print(f"[broker] cannot verify dataset {ref!r}; keeping it as-is",
                      flush=True)
                out.append(ref)
            else:
    ...
    def _dataset_exists(self, ref: str) -> bool | None:
        """Does OWNER/slug actually resolve on Kaggle?

        Three-valued on purpose. True/False are verdicts; None means the
        question could not be asked — no credentials, no network, CLI error.
        Collapsing None into False made an unauthenticated broker silently
        strip the wheel dataset every kernel mounts, which then fails minutes
        later as "wheels not found under /kaggle/input".
        """
        import subprocess
        ...
        try:
            r = subprocess.run(
                [_kaggle_bin(), "datasets", "list", "-s", ref.split("/")[-1]],
                capture_output=True, text=True, timeout=90)
        except Exception:  # noqa: BLE001 - unreachable Kaggle is not a verdict
            return None
        if r.returncode != 0:
            return None
        return any(line.split()[0] == ref
                   for line in r.stdout.splitlines() if line.split())
    ...
    def _submit_kernel(
        ...
        try:
            ...
            # make_metadata enforces Kaggle's title/id length limit. Always use
            # the exact ref written to disk for polling and submission, even if
            # a future caller supplies an overlong pre-normalized candidate id.
        ...
        if self.kernel_version_budget > 0:
            used = self.resource_gate.pushes_used(self.slug)
            if used >= self.kernel_version_budget:
                self.resource_gate.release(lease_id)
                return SubmitResult(
                    False, "rejected",
                    f"notebook version budget exhausted: {used}/"
                    f"{self.kernel_version_budget} pushes already spent",
                )
        # Booked before the call: a push that errors still spends a version.
        self.resource_gate.record_push(self.slug)
        ...
        # Feed the real wall-clock back so the next estimate is data, not a
        # guess. Kaggle's runtime is the closest thing to the grader's budget.
        if status == "complete":
            # Record the wall-clock we actually measured, never `gpu_seconds`.
            # estimate_gpu_seconds() falls back to the *declared* budget when a
            # log has no parseable duration, so feeding it back here taught the
            # estimator its own guess: task 5 logged [28, 28, 28] minutes for
            # kernels that really took 2.1-3.2, and four submissions were then
            # blocked as "not enough time for estimated 28.0 minute kernel"
            # with 16 minutes still on the clock.
            measured = (time.monotonic() - push_started) / 60.0
            if measured > 0:
                bucket = self._observed_minutes.setdefault(accelerator, [])
                bucket.append(measured)
                del bucket[:-8]  # keep the last 8 observations
        if status != "complete":
            retryable_mount = (
