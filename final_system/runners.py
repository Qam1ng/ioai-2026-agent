...
class ClaudeSubscriptionRunner(_ProcessRunner):
    ...
    def environment(self, workdir: Path) -> dict[str, str]:
        ...
        if not max_output_tokens.isdigit() or int(max_output_tokens) <= 0:
            raise ValueError(
                "IOAI_CLAUDE_MAX_OUTPUT_TOKENS must be a positive integer"
            )
...
async def run_lane_loop(
    *, lane: str, runner, prompt: str, continuation: str, workdir: Path,
    ...
    for round_id in range(1, max_rounds + 1):
        ...
        if session_id is None and result.exit_code not in (0, None):
        ...
        # Retrying a continuation prompt cannot repair a nonexistent session.
        # Keep one retry for a transient startup race, then fail the lane fast
        # instead of burning all configured rounds in a few seconds.
        if bootstrap_failures >= 2:
            break
