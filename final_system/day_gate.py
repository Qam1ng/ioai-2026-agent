...
class DayResourceGate:
    ...
    def floor_done(self, slug: str) -> bool:
        """Has this day-group already produced a floor submission for `slug`?

        The pool file outlives any single run, so a continuation run can tell
        that the floor is already paid for. Without this the Broker labelled
        the first submission of every run "floor" — task 5's continuation run
        spent a slot re-paying a floor the day had already satisfied.
        """
        if not self.floor_group_id:
            return False
        state = read_json(self.state_path, self._new_state())
        group = state.get("floor_groups", {}).get(self.floor_group_id, {})
        return bool(group.get("floors", {}).get(slug))

    @property
    def _versions_path(self) -> Path:
        return self.root / "kernel_versions.json"

    def pushes_used(self, slug: str) -> int:
        return int(read_json(self._versions_path, {}).get(slug, 0))

    def record_push(self, slug: str) -> int:
        """Count one `kernels push`. Task 5 capped notebook *versions*, not
        submissions, and every push spent one even when the run failed."""
        with locked(self._versions_path):
            data = read_json(self._versions_path, {})
            data[slug] = int(data.get(slug, 0)) + 1
            atomic_json(self._versions_path, data)
            return data[slug]
