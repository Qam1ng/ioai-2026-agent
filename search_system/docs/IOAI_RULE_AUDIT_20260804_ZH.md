...
1. 单日三题不是同时开放。每题先有 5 分钟加入缓冲，供人类点击 Join competition 并粘贴官方 starter prompt；随后 Agent 最多运行 2 小时；再有 10 分钟空档，进入下一题的 5 分钟缓冲。因而相邻两道 2 小时解题窗口相隔 15 分钟，但纯休息只有 10 分钟。Prompt 原来的“15 分钟”并非错误，现已改成不易误解的完整表述。
...
- 每次 Agent 调用使用 Runner 注入的绝对截止时间，不从“六小时”“两小时”或本文推算实际预算。
