...
Search 模块整体受硬时间盒约束，建议不超过单题窗口的 20%（2 小时窗口即约 25 分钟），由 runner 强制终止。依据是实测赛制经济学：Kaggle 提交吞吐约 4–5 分钟/次、并发上限 2，单题窗口内 wall-clock 时间近似等于提交次数——search 模块每多占 10 分钟，约等于烧掉 2 个提交槽。
...
正式赛分两个比赛日。公开规则文件将每个比赛日描述为一个包含三道题的六小时 submission window；最新参赛者公告进一步说明，实际时序为三场互不重叠的 Kaggle competition：每题先开放 5 分钟供人类操作员点击 Join competition 并粘贴官方 starter prompt，随后 Agent 获得最多 2 小时；本题结束后有 10 分钟空档，再进入下一题的 5 分钟加入缓冲。因此相邻两道 2 小时解题窗口之间相隔 15 分钟，但其中只有 10 分钟是纯休息。任何 Agent 都必须以当前调用注入的绝对截止时间为准，绝不能把“六小时”误认为当前单题拥有六小时。每道题最多允许 50 次提交。提交额度是一等测量资源：在小样本或含近重复样本的数据上，本地验证分数可能与排行榜弱相关甚至符号反转（本系统在音频任务族上实测过榜上 +0.096 的改动在本地 5-fold CV 中显示为 −0.127），此时排行榜是唯一可信的评判。提交应当按策略刻意地花，而不是按直觉节省。参赛组织在自己的硬件上运行 Agent；最终 solution code 通过 Kaggle 提交，并在统一的 Kaggle GPU 环境中执行和计分。计分 kernel 必须关闭互联网、使用题目挂载的固定依赖资产（包括组织者提供的 wheel 数据集与安装脚本）及基础环境，在单 GPU 和 kernel 时限内完成全部训练与推理。额外数据集或模型默认禁止；只有当前题面明确允许时才可例外。任何不满足当前题 kernel 契约的方法，无论论文分数多高，对本比赛都不可用。
