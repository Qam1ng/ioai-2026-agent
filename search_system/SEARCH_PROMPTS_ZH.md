...
1. 阶段 A 开始后最多只用前 45% 时间做盘点和推导；不得因为想穷尽 metric、模型 metadata 或全量统计而延迟落盘。
2. 在截止时间过去 60% 前，必须先在 `{analysis_stage_a_dir}` 写出三个最小合格文件：`ASSET_MAP.md`、`TASK_ANALYSIS_V1.md`、`RESEARCH_CHARTERS.md`。不确定的内容用 `[UNRESOLVED]`，不得留到文件完全不存在。
3. 三个文件落盘后才允许继续统计、联网核对和补充；后续以原子替换方式改进它们。
4. 若剩余时间不足 40%，立即停止新搜索、大型统计和延伸分析，只做文件完整性、证据路径和 `research_plan` 语法自检。
5. 资产数量很大时只保留汇总计数和代表性样本；不要把数万行 `find` 输出或完整二进制 metadata 送回上下文。
