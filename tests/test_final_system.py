...
def make_evaluation(root: Path) -> Path:
    ...
    (source / "contract_evidence.json").write_text(
        json.dumps({
            ...
            "split_rationale": "deterministic three-fold unit test",
        }),
        encoding="utf-8",
    )
...
def test_config_is_formal_duration_agnostic() -> None:
    ...
    # Quota numbers are tuned per competition (task 5 ran 15/3), so pinning
    # exact values here just turns every task into a red test. Assert the
    # relationships that must hold whatever the organisers allow.
    assert config.run.max_submissions > 0
    assert 0 < config.run.final_reserve < config.run.max_submissions
    assert config.run.initial_calibrations < config.run.max_submissions
    assert config.run.kernel_version_budget >= 0
    assert config.claude.model == "claude-opus-5"
    assert config.claude.effort == "max"
    assert config.claude.anthropic_base_url.endswith("/anthropic")
    assert config.codex.model == "gpt-5.6-sol"
    assert config.codex.effort == "max"
    assert config.search.analyst_backend == "codex"
    assert config.search.research_backends == ("codex",) * 4
    assert config.search.enabled is False
    assert config.hearsay.enabled is True
    assert config.selection.model == "gpt-5.6-sol"
    assert config.selection.effort == "max"
...
def test_evaluation_contract_can_replace_controller_precreated_empty_mount(
    ...
    (source / "folds.json").write_text(
        json.dumps({
            "ids": ["a", "b", "c"], "fold": [0, 1, 2], "labels": [0, 1, 0],
            "scheme": "kfold",
        }),
        encoding="utf-8",
    )
...
def test_submission_csv_derives_task2_template_from_test_observations(
    tmp_path: Path,
) -> None:
    assets = tmp_path / "assets"
    test = assets / "data" / "test"
    test.mkdir(parents=True)
    (test / "observations.json").write_text(
        json.dumps([{"id": "robot-b"}, {"id": "robot-a"}]), encoding="utf-8"
    )
    candidate = tmp_path / "submission.csv"
    candidate.write_text(
        "id,prediction\nrobot-b,0.25\nrobot-a,0.75\n", encoding="utf-8"
    )
    valid = validate_submission_csv(assets, candidate)
    assert valid["valid"] is True
    assert valid["sample_submission"].startswith("derived:")
    candidate.write_text(
        "id,prediction\nrobot-a,0.25\nrobot-b,0.75\n", encoding="utf-8"
    )
    invalid = validate_submission_csv(assets, candidate)
    assert invalid["valid"] is False
    assert any("IDs/order" in error for error in invalid["errors"])


def test_submission_csv_derives_task1_template_from_test_prefix(
    tmp_path: Path,
) -> None:
    assets = tmp_path / "assets"
    test = assets / "Archive" / "test"
    test.mkdir(parents=True)
    (test / "prefix.csv").write_text(
        'filename,prefix\n"16","[5,2]"\n"2","[3,2]"\n',
        encoding="utf-8",
    )
    candidate = tmp_path / "submission.csv"
    ...
    valid = validate_submission_csv(assets, candidate)
    assert valid["valid"] is True
    assert valid["sample_submission"].endswith("test/prefix.csv")
    candidate.write_text(
        'filename,prediction\n16,"[1,0]"\n2,"[0,1]"\n',
        encoding="utf-8",
    )
...
def test_codex_only_config_routes_enabled_roles_to_codex(tmp_path: Path) -> None:
    assets = make_assets(tmp_path)
    config = SystemConfig.load(
        ROOT / "configs/final_agent_system.toml", repo_root=ROOT
    )
    controller = FinalController(
        ...
        duration_minutes=1, competition_mode="practice",
        kaggle_user="", live=False, run_id="codex-only",
    )

    assert config.search.analyst_backend == "codex"
    assert config.search.research_backends == ("codex",) * 4
    assert config.hearsay.enabled is True
    assert isinstance(controller._selection_runner(), OpenRouterCodexRunner)
    assert isinstance(controller._secondary_codex_runner(), OpenRouterCodexRunner)
...
def test_kernel_adapter_validates_downloaded_output_before_submission(
    ...
    monkeypatch.setattr(
        ...
        lambda _path, **_kwargs: (True, 7, "ok"),
    )
    monkeypatch.setattr(
    ...
    # This test is about output validation, not dataset resolution. Pin the
    # lookup so it does not depend on whether "owner/dataset" exists on Kaggle.
    monkeypatch.setattr(KaggleAdapter, "_dataset_exists", lambda self, ref: True)
...
def test_bwrap_reveals_lane_and_assets_after_hiding_session(tmp_path: Path) -> None:
    session = tmp_path / "session"
    lane = session / "lanes" / "codex"
    assets = lane / "OFFICIAL_ASSETS"
    control = session / "control"
    for path in (assets, control):
        path.mkdir(parents=True)
    (assets / "task.txt").write_text("official\n", encoding="utf-8")
    (control / "broker_state.json").write_text("secret\n", encoding="utf-8")
    probe = control / "security" / "sandbox_probe_secret"
    sandbox = build_agent_sandbox(
        probe_secret=probe,
        protected_write_roots=(session,),
        writable_roots=(lane,),
        read_only_roots=(assets,),
        protected_read_roots=(session,),
        readable_roots=(lane, assets),
    )
    verify_agent_sandbox(sandbox, probe_secret=probe)
    readable = subprocess.run(
        sandbox.wrap(["/bin/cat", str(assets / "task.txt")]),
        capture_output=True, text=True, timeout=10,
    )
    writable = subprocess.run(
        sandbox.wrap([
            "/bin/sh", "-c", "/bin/echo candidate > \"$1\"", "sh",
            str(lane / "candidate.txt"),
        ]),
        capture_output=True, text=True, timeout=10,
    )
    immutable = subprocess.run(
        sandbox.wrap([
            "/bin/sh", "-c", "/bin/echo tamper > \"$1\"", "sh",
            str(assets / "task.txt"),
        ]),
        capture_output=True, text=True, timeout=10,
    )
    ...
    assert readable.returncode == 0 and readable.stdout.strip() == "official"
    assert writable.returncode == 0 and (lane / "candidate.txt").is_file()
...
def test_controller_dry_run_closes_full_wiring_without_model_or_kaggle(
    ...
    class FakeSelectionRunner:
        async def run(
            ...
            return AgentResult(
                agent_id=agent_id, role=role, workdir=workdir,
                trace_path=trace, exit_code=0, timed_out=False,
                duration_s=0.01, resolved_model="gpt-5.6-sol",
                result_text=json.dumps({
    ...
    monkeypatch.setattr(
        controller, "_selection_runner", lambda: FakeSelectionRunner()
    )

    monkeypatch.setattr(
        controller, "_secondary_codex_runner",
        lambda **_kwargs: FakeRunner("claude"),
    )
    ...
    assert status["selection_manager"]["model"] == "gpt-5.6-sol"
